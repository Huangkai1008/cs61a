...
def planet(size):
    ...
    The first element is the string 'planet' and the second element is its size.
...
def balanced(m):
    ...
    return all(
        [
            length(left_arm) * total_weight(end(left_arm))
            == length(right_arm) * total_weight(end(right_arm)),
            balanced(end(left_arm)),
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert upper_bound(y) != 0 and lower_bound(y) != 0 and 1 / upper_bound(y) <= 1 / lower_bound(y)
